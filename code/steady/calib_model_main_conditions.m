function F = calib_model_main_conditions(x, param)

% unpack parameters
target_for_N = param.target_for_N;
target_for_E_or_C_E = param.target_for_E_or_C_E;
target_for_pgamma = param.target_for_pgamma;
target_for_palpha = param.target_for_palpha;
target_for_pebar = param.target_for_pebar;
...
pchi = x(count); count = count + 1;
xi_E = x(count); count = count + 1;
palpha = x(count); count = count + 1;
pgamma = x(count); count = count + 1;
pebar = x(count); count = count + 1;
...
if round(target_for_pgamma, 4) == 0.0
...
if round(target_for_palpha, 4) == 0.0
...
if round(target_for_pgamma, 4) == 0.0 || round(target_for_pebar, 4) == 0.0
...
% targets
F(count) = (N) - (target_for_N); count = count + 1;
if round(target_for_palpha, 4) == 0.0
    F(count) = (C_E) - (target_for_E_or_C_E); count = count + 1;
else
    F(count) = (E) - (target_for_E_or_C_E); count = count + 1;
end

if round(target_for_pgamma, 4) == 0.0
    F(count) = (pgamma) - (0.0); count = count + 1;
else
    F(count) = (sh_E_in_C) - (100 * target_for_pgamma); count = count + 1;
end

if round(target_for_palpha, 4) == 0.0
    F(count) = (palpha) - (0.0); count = count + 1;
else
    F(count) = (sh_E_in_Y) - (100 * target_for_palpha); count = count + 1;
end

if round(target_for_pgamma, 4) == 0.0 || round(target_for_pebar, 4) == 0.0
    F(count) = (pebar) - (0.0); count = count + 1;
else
    F(count) = (sh_subsistence) - (100 * target_for_pebar); count = count + 1;
end

if ~isnan(target_for_inequality)
    F(count) = (inequality) - (target_for_inequality); count = count + 1;
else
    F(count) = (Tbar) - (0.0); count = count + 1;
end
